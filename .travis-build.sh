...
then
    pkgname="$(rpmspec -q --qf="cdo-%{version}-%{release}\n" cdo.spec | head -n1)"
    mkdir -p ~/rpmbuild/{BUILD,BUILDROOT,RPMS,SOURCES,SPECS,SRPMS}
    cp cdo.spec ~/rpmbuild/SPECS/
    spectool -g -R -S ~/rpmbuild/SPECS/cdo.spec
    set +x
    rpmbuild -ba ~/rpmbuild/SPECS/cdo.spec 2>&1 | pv -q -L 3k
